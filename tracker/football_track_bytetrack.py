...
from yolox.data.data_augment import preproc_default
...
class Football_bytetrack():
    ...
    def inference(self, frame):
        ...
        if outputs[0] is not None:
            ...
            res = "".join(results)
            df = pd.read_csv(io.StringIO(res), sep=",", names=[
                             "id", "x", "y", "w", "h", "score", "label", "x_center", "y_center", "color"])
            timer.toc()
