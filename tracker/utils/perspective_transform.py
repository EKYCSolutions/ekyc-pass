...
class Perspective_Transform():
    def __init__(self):
        ...
        self.deep_model_directory = os.path.join(
            self.current_directory, "deep_network.pth")
    ...
    def homography_matrix(self, image):
        ...
        data = sio.loadmat(os.path.join(
            self.current_directory, "data/worldcup2014.mat"))
        ...
        flann = pyflann.FLANN()
        result, _ = flann.nn(
            self.database_features, test_features[self.query_index], 1, algorithm="kdtree", trees=8, checks=64)
        ...
        Warp_img = cv2.warpPerspective(seg_map, np.linalg.inv(
            refined_h), (115, 74), borderMode=cv2.BORDER_CONSTANT)
    ...
    def initialize_deep_feature(self):
        ...
        if os.path.isfile(self.deep_model_directory):
            checkpoint = torch.load(
                self.deep_model_directory, map_location=lambda storage, loc: storage)
