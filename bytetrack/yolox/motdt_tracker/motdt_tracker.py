...
class OnlineTracker(object):
    ...
    def update(self, output_results, img_info, img_size, img_file_name):
        img_file_name = os.path.join(
            get_yolox_datadir(), 'mot', 'train', img_file_name)
        ...
        detections = [STrack(tlwh, score, from_det=True)
        ...
        if len(detections) > 0:
            nms_out_index = torchvision.ops.batched_nms(
                ...
                torch.from_numpy(scores.reshape(-1)
                                 ).to(torch.from_numpy(rois).dtype),
        ...
        for itracked, idet in matches:
            tracked_stracks[itracked].update(
                detections[idet], self.frame_id, image)
        ...
        for ilost, idet in matches:
            ...
            det = detections[idet]
            track.re_activate(det, self.frame_id, image,
