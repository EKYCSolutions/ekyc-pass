...
class BYTETracker(object):
    ...
    def update(self, output_results, img_info, img_size):
        ...
        class_label = output_results[:, 6]
        class_label = class_label[remain_inds]
        class_label = [labels[i+1] for i in class_label]

        class_label_second = output_results[:, 6][inds_second]
        class_label_second = [labels[i+1] for i in class_label_second]

        if len(dets) > 0:
            '''Detections'''
            detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s, label) for
                          (tlbr, s, label) in zip(dets, scores_keep, class_label)]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, label) for
                                 (tlbr, s, label) in zip(dets_second, scores_second, class_label_second)]
        else:
            detections_second = []
        ...
        return output_stracks, class_label, bboxes
